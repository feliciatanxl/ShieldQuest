...
export function Die({
  ...
  return (
    <span
      aria-hidden="true"
      className={`grid shrink-0 grid-cols-3 gap-[2.5px] rounded-[16px] border-2 border-navy-950/20 bg-white p-1.5 shadow-[0_6px_12px_rgba(0,0,0,0.35),inset_0_-3px_0_rgba(11,37,69,0.18)] ${className} ${
      ...
      <div className="rounded-[24px] border-2 border-white/20 bg-navy-950/90 p-2 shadow-[0_16px_36px_rgba(0,0,0,0.9),inset_0_1px_0_rgba(255,255,255,0.15)] backdrop-blur-md">
        {showing && value !== null ? (
          <div
            ...
            aria-live="polite"
          >
            <span className="text-[12px] font-black uppercase tracking-[0.2em] text-[var(--sq-earned)]">
              Rolled
            </span>
            ...
            <Sparkles className="h-5 w-5 text-[var(--sq-earned)] animate-pulse" />
          </div>
        ) : (
          <button
            type="button"
            ...
            className="group relative flex min-h-[64px] w-full items-center justify-between gap-3 overflow-hidden rounded-[16px] border-b-4 border-amber-800 bg-gradient-to-r from-amber-400 via-amber-500 to-amber-400 px-5 text-[var(--sq-ink)] shadow-xl shadow-amber-500/25 transition active:translate-y-1 active:border-b-0 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {/* Shimmer Highlight */}
            <span
              aria-hidden="true"
            ...
            <div className="flex flex-col items-start leading-none">
              <span className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--sq-ink)]/75">
                {rolling ? 'Advancing…' : 'Your Turn'}
              </span>
              <span className="mt-1 text-[20px] font-black uppercase tracking-wider text-[var(--sq-ink)]">
            ...
            <div className="flex items-center gap-2">
              <div className="flex h-12 w-12 items-center justify-center rounded-[10px] bg-navy-950/10 shadow-inner">
                <Die
