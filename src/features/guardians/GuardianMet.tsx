    ...
    >
      {guardian && (
        <div className="animate-slide-up px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-6 text-center">
          <p className="inline-flex items-center gap-1.5 rounded-full bg-[var(--sq-safe)]/15 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.2em] text-[var(--sq-safe)]">
          ...
          <GuardianPlate
            guardian={guardian}
            className="animate-pop mx-auto mt-4 h-24 w-24 rounded-[24px] text-3xl"
            tone="amber"
          />

          <h2
            id="guardian-met-title"
            className="mt-4 text-[30px] font-extrabold uppercase leading-none tracking-tight text-[var(--sq-ink)]"
          >
            {guardian.name}
          </h2>

          <p className="mt-2 inline-flex items-center gap-2 rounded-[10px] border border-line bg-surface-sunk px-3 py-1.5 text-[12.5px] font-bold text-[var(--sq-ink)]">
            <span
              aria-hidden="true"
          ...
          <button
            type="button"
            onClick={onContinue}
            className="mt-5 flex min-h-[52px] w-full items-center justify-center gap-2 rounded-[10px] border-b-4 border-leaf-800 bg-leaf-700 px-4 text-[15px] font-extrabold uppercase tracking-[0.08em] text-white transition hover:bg-leaf-600 active:translate-y-[3px] active:border-b-0"
          >
            Continue
