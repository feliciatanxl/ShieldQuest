...
import { TARGET_GROUP_AGE, type GroupDecisionSignal } from './types.ts';
...
export function GroupDecisionSignalPanel({ signals }: { signals: GroupDecisionSignal[] }) {
  const reconsideredAverage = signals.length
    ? Math.round(signals.reduce((sum, s) => sum + s.reconsideredPct, 0) / signals.length)
    : 0;
              ...
              {signals.map((signal) => {
                ...
                return (
                  <tr key={signal.id} className="hover:bg-surface-sunk">
                    <td className="px-4 py-3 font-semibold text-navy-900">{signal.question}</td>
                    ...
                    <td className="px-4 py-3 text-center tabular-nums font-bold text-leaf-700">
                      {signal.finalSafePct}%{' '}
                      {delta > 0 && (
                        <span className="text-[11px] font-semibold text-leaf-600">(+{delta})</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center tabular-nums font-bold text-civic-700">
                      {signal.reconsideredPct}%
                    </td>
                    <td className="px-4 py-3 text-[12.5px] text-ink-muted">“{signal.topFactor}”</td>
                  </tr>
                );
              })}
            </tbody>
