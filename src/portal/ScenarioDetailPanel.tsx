import { ClipboardCheck, Minus, PencilLine, TrendingDown, TrendingUp, Zap } from 'lucide-react';
import { REVIEW_THRESHOLD, STATUS_STYLES } from './ScenarioTable.tsx';
import { COMPETENCY_LABEL, COMPETENCY_LETTER } from '../game/types.ts';
import { ScenarioResponsesPanel } from './ScenarioResponsesPanel.tsx';
import { ScenarioReviewPanel } from './ScenarioReviewPanel.tsx';
...
const OBJECTIVES: Record<string, string> = {
  ...
  'E-Commerce Scam': 'Check a seller through a channel you already trust before any money moves.',
  ...
  'Job Scam': 'Test an offer against what a real employer would actually need from you.',
  'Vape Possession':
...
export function ScenarioDetailPanel({
  ...
  const needsReview =
    row.status === 'LIVE' && row.responses > 0 && row.safeDecisionRate < REVIEW_THRESHOLD;
    ...
    <div className="flex h-full flex-col bg-surface">
      ...
      >
        {row.status === 'LIVE' && row.responses > 0 && (
          <section className="rounded-[16px] border border-line bg-surface-sunk p-4">
            ...
            <div className="mt-2 flex items-baseline gap-3">
              ...
              <Trend current={row.safeDecisionRate} previous={row.previousSafeDecisionRate} />
            </div>
            <p className="mt-1 text-[12.5px] leading-relaxed text-ink-muted">
              Based on{' '}
              <strong className="font-bold text-navy-900">{row.responses.toLocaleString()}</strong>{' '}
            ...
            {needsReview && (
              <p className="mt-2 rounded-[6px] border border-amber-200 bg-amber-50 p-2.5 text-[12px] leading-relaxed text-amber-700">
                Below the {REVIEW_THRESHOLD}% review threshold. Review the situation wording and
                debrief guidance.
              </p>
            )}
          </section>
        ...
        <section>
          ...
          <p className="mt-1.5 text-[14px] leading-relaxed text-ink">{objective}</p>
        </section>
