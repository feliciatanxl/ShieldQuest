...
import { COMPETENCY_LABEL, type Competency } from '../game/types.ts';
...
const EXAMPLE: FlashMissionDraft = {
  ...
  prompt: 'An online recruiter says you can earn commission by receiving and forwarding payments.',
  choices: [
...
const labelCls = 'block text-[11px] font-bold uppercase tracking-[0.12em] text-ink-muted';
const fieldCls =
...
export function FlashMissionPanel({
  ...
  const set = <K extends keyof FlashMissionDraft>(key: K, value: FlashMissionDraft[K]) =>
  ...
  const selectedGuardian = GUARDIANS.find((g) => g.id === draft.guardianId) ?? GUARDIANS[0]!;
      ...
      <div className="flex h-full min-h-0 flex-col bg-surface">
        ...
        <div className="thin-scroll min-h-0 flex-1 space-y-4 overflow-y-auto p-6">
          <div className="rounded-[16px] border border-line bg-surface-sunk p-4">
            ...
            <h3 className="mt-2 text-[17px] font-extrabold text-navy-900">{deployed.title}</h3>
            <p className="mt-0.5 text-[13px] text-ink-muted">
              {deployed.category} · {deployed.targetGroup}
            </p>
            <p className="mt-3 text-[14px] leading-relaxed text-ink">{draft.prompt}</p>
          </div>

          <div className="rounded-[16px] border border-line bg-surface p-4">
            <p className={labelCls}>Debrief note</p>
            <p className="mt-1 text-[13px] leading-relaxed text-ink-muted">{draft.debrief}</p>
          </div>
        </div>
      ...
      <div className="thin-scroll min-h-0 flex-1 space-y-6 overflow-y-auto px-6 py-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                id="fm-status"
                value={draft.status}
                onChange={(e) => set('status', e.target.value as FlashMissionDraft['status'])}
                className={fieldCls}
              >
        ...
        <FormSection index={3} title="Decision choices">
          <p className="text-[12px] text-ink-soft">
            Mark which option represents the intended learning choice. This is author-facing only.
          </p>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="mt-1.5 flex items-center gap-3">
                <img
                  src={guardianArt(selectedGuardian.id)}
                  alt=""
                  aria-hidden="true"
                  className="h-10 w-10 shrink-0 rounded-[10px] bg-navy-900 object-contain p-1"
                />
                <select
                  id="fm-guardian"
