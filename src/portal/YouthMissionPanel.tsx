...
import { YouthMissionStatusChip } from './YouthMissionQueue.tsx';
...
export function YouthMissionPanel({
  ...
  onDecide: (id: string, decision: YouthMissionDecision, note: string) => void;
  onClose: () => void;
}) {
  ...
  return (
    <div className="flex h-full flex-col bg-surface">
      <header className="border-b border-line bg-surface-sunk px-6 py-5 pr-14">
        ...
        <p className="mt-0.5 text-[13px] text-ink-muted">{submission.category}</p>
      </header>

      <div className="thin-scroll flex-1 space-y-5 overflow-y-auto px-6 py-5">
        <Field label="The idea, as submitted">
          <p className="text-[14px] leading-relaxed text-ink">{submission.summary}</p>
        </Field>

        <Field label="What the submitter says it teaches">
          <p className="text-[14px] leading-relaxed text-ink">{submission.intendedLesson}</p>
        </Field>
        ...
        <Field label="Proposed audience band">
          <p className="inline-flex items-center gap-1.5 text-[14px] font-semibold text-ink">
            ...
            <span className="text-ink-muted">{TARGET_GROUP_AGE[submission.suggestedBand]}</span>
          </p>
        </Field>
        ...
        <div className="rounded-[16px] border border-line bg-surface-sunk p-3.5 text-[12px] text-ink-muted">
          ...
          <p className="mt-1">
            ...
            {submission.submitterBand}). No personal data, school, class or contact detail exists.
          </p>
        </div>
      </div>
