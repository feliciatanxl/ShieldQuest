  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-[10px] border border-leaf-200 bg-leaf-50 px-5 py-10 text-center">
        <CheckCircle2 className="mx-auto h-6 w-6 text-leaf-700" aria-hidden="true" />
        <p className="mt-2 text-[15px] font-bold text-leaf-700">Nothing is awaiting review</p>
            ...
            <div className="p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <h3 className="mt-1 text-[17px] font-extrabold text-navy-900">{row.title}</h3>
                </div>
                ...
                <div className="flex items-baseline gap-3 rounded-[6px] border border-line bg-surface-sunk px-3 py-1.5">
                  ...
                  <div className="border-l border-line pl-3">
                    ...
                    >
                      ...
                      {!hasPrevious ? 'New' : diff === 0 ? '0' : `${diff > 0 ? '+' : ''}${diff}`}
                    </p>
                  </div>
                </div>
              </div>

              <p className="mt-3 text-[13px] leading-relaxed text-ink-muted">{reasonFor(row)}</p>
