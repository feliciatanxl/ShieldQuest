/* =============================================================================
 * Portal types — the facilitator-facing vocabulary.
 * -----------------------------------------------------------------------------
 * Ported from v1 (`types/admin.ts`, tag `v1-archive`). The shapes are unchanged
 * because they encode proposal commitments, not implementation detail:
 *
 *   - `safeDecisionRate` measures a SCENARIO, never a participant. Every field
 *     in `AdminScenarioRow` is content-level; there is no per-youth record in
 *     this file, and there must never be one (§4 data minimisation).
 *   - `YouthMissionSubmission` carries a pseudonym and a band and nothing else.
 *     The strongest decision a reviewer can take is `CONVERTED`, which produces
 *     a DRAFT — there is no publish path out of the youth queue.
 *   - `KpiStatus` exists so the portal can say which numbers are demonstrated,
 *     which are simulated and which are merely planned. The funded pilot has
 *     not run; nothing here may imply that it has.
 * ========================================================================== */

import type { Competency } from '../game/types.ts';
...
export type SimulatedCohortId = 'primary' | 'secondary' | 'tertiary' | 'community';
...
export type YouthMissionStatus = 'AWAITING_REVIEW' | 'CHANGES_REQUESTED' | 'CONVERTED' | 'REJECTED';
...
export type YouthMissionDecision = Exclude<YouthMissionStatus, 'AWAITING_REVIEW'>;
...
export type AdminSection =
  'overview' | 'sessions' | 'library' | 'builder' | 'review' | 'youth' | 'insights' | 'resources';
