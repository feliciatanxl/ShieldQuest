...
export const PILOT_KPIS: PilotKpi[] = [
  ...
  },
  {
    ...
    measures:
      'Whether the delayed cost of a decision can be named while the reward is still visible.',
...
export const MOCK_YOUTH_SUBMISSIONS: YouthMissionSubmission[] = [
  ...
  },
  {
    ...
    intendedLesson: 'Submitter’s stated aim was to use real examples so the content stays current.',
...
export function derivePortalSummary(rows: AdminScenarioRow[]): PortalSummary {
  ...
  const average = scored.length
    ? Math.round(scored.reduce((sum, r) => sum + r.safeDecisionRate, 0) / scored.length)
    : 0;
  return {
