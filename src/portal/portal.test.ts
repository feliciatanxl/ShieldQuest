 ...
 * Portal tests.
 * -----------------------------------------------------------------------------
 * Ported from v1 `features/admin/admin.test.ts`. These are commitment tests,
 * not unit tests: each one pins a promise the proposal makes about the portal.
 *   - A youth submission can only ever become a DRAFT (§5 moderation).
 *   - The six pilot KPIs declare honestly which are demonstrated, which are
 *     simulated and which are merely planned — the funded pilot has not run.
 *   - The safe decision rate is content-level; nothing here is per-participant.
 * Run with the engine tests: `npm test`.
 * ========================================================================== */
...
import { applyScenarioFilters, EMPTY_FILTERS } from './ScenarioFilters.tsx';
import { ADMIN_NAV } from './AdminChrome.tsx';
import type {
  AdminScenarioRow,
  FlashMissionDraft,
...
test('derivePortalSummary calculates active, participants, average, and review counts', () => {
  ...
  const liveScored = MOCK_ADMIN_SCENARIOS.filter((r) => r.status === 'LIVE' && r.responses > 0);
  assert.equal(summary.activeScenarios, 15);
  assert.equal(summary.participants, 1248);
  assert.ok(summary.averageSafeDecisionRate > 0);
  assert.equal(summary.needsReview, liveScored.filter((r) => r.safeDecisionRate < 60).length);
...
test('ADMIN_NAV includes all 8 required sections including sessions, builder, and resources', () => {
