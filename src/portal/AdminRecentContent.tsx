...
import { STATUS_STYLES } from './ScenarioTable.tsx';
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-[10px] border border-line bg-surface px-4 py-8 text-center">
        <p className="text-[14px] font-semibold text-ink-muted">No content yet.</p>
      </div>
    );
  }
          ...
          <li key={row.id}>
            ...
            >
              <span className="flex min-w-0 flex-1 items-center gap-2">
                <span className="truncate text-[14px] font-bold text-navy-900">{row.title}</span>
                {row.isFlashMission && (
