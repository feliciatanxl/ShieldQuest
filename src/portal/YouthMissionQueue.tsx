import { CircleSlash, FileEdit, PencilLine, ShieldAlert } from 'lucide-react';
...
export function YouthMissionStatusChip({ status }: { status: YouthMissionStatus }) {
  return (
    <span
        ...
        <li key={submission.id}>
          ...
          >
            ...
            <p className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-ink-soft">
              <span className="font-semibold text-ink-muted">{submission.category}</span>
              <span aria-hidden="true">·</span>
              <span>
                {submission.suggestedBand}{' '}
                <span className="tabular-nums">{TARGET_GROUP_AGE[submission.suggestedBand]}</span>
              </span>
              <span aria-hidden="true">·</span>
