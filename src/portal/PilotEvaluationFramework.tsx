...
import { KPI_STATUS_LABEL, type KpiStatus, type PilotKpi } from './types.ts';

const STATUS_STYLE: Record<KpiStatus, { skin: string; Icon: typeof ShieldCheck }> = {
  DEMONSTRATED: {
    ...
    <ol className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
      {kpis.map((kpi) => (
        ...
        >
          ...
          <h3 className="mt-2 text-[15px] font-bold text-navy-900">{kpi.name}</h3>

          <p className="mt-1 text-[13px] leading-relaxed text-ink">{kpi.measures}</p>

          <div className="mt-auto border-t border-line pt-3">
            <span className="text-[11px] font-bold uppercase tracking-wide text-ink-soft">
              Measurement method
            </span>
            <p className="mt-0.5 text-[12px] leading-relaxed text-ink-muted">{kpi.method}</p>
          </div>
        </li>
      ))}
    </ol>
