...
import type { SkillCoverage } from './types.ts';
  ...
  return (
    <div className="rounded-[16px] border border-line bg-surface p-4">
      <ul className="space-y-3">
        {rows.map((row) => (
          <li key={row.competency}>
            ...
            <div className="mt-1.5 h-2.5 overflow-hidden rounded-full bg-line" aria-hidden="true">
              <div
      ...
      <p className="mt-4 border-t border-line pt-3 text-[12px] leading-relaxed text-ink-muted">
        Skill coverage measures how thoroughly our published scenarios teach each prevention
        competency. Aggregated across content only.
      </p>
    </div>
  );
}
