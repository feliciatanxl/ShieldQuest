...
import type { AdminScenarioRow, AdminScenarioStatus } from './types.ts';
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-[10px] border border-line bg-surface px-5 py-12 text-center">
        <p className="text-[14px] font-semibold text-ink-muted">No scenarios match this view.</p>
      </div>
    );
  }
        ...
        <table className="w-full min-w-[920px] border-collapse text-left">
          ...
          <tbody className="divide-y divide-line">
            {rows.map((row) => {
              ...
              return (
                <tr key={row.id} className={isNew ? 'bg-civic-50' : 'hover:bg-surface-sunk'}>
                  <th scope="row" className="px-4 py-3.5 text-left font-normal">
                    <span className="flex items-center gap-2">
                      {onSelect ? (
                        ...
                        <span className="text-[14px] font-bold text-navy-900">{row.title}</span>
                      )}
                      {row.isFlashMission && (
                  ...
                  <td className="px-4 py-3.5 text-[13px] text-ink">{row.category}</td>
                  <td className="px-4 py-3.5 text-[13px] text-ink-muted">{row.targetGroup}</td>
                  ...
                  <td className="px-4 py-3.5 text-[12px] text-ink-soft">
                    ...
                    <span className="block text-[11px] text-ink-soft">{row.updatedBy}</span>
                  </td>
                </tr>
              );
            })}
