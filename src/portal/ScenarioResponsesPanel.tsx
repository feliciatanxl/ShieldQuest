...
import { bandBreakdown, choiceBreakdown } from './scenario-insights.ts';
import { REVIEW_THRESHOLD } from './ScenarioTable.tsx';
import type { AdminScenarioRow } from './types.ts';
  ...
  return (
    <div
      className={`rounded-[10px] border p-3.5 ${
        tone === 'attention' ? 'border-amber-200 bg-amber-50' : 'border-line bg-surface-sunk'
      }`}
    >
      <p className="text-[11px] font-bold uppercase tracking-wider text-ink-muted">{label}</p>
      <p className="mt-1 text-[22px] font-black tabular-nums leading-none text-navy-900">{value}</p>
...
export function ScenarioResponsesPanel({ row }: { row: AdminScenarioRow }) {
  ...
  const topDistractor = choices.filter((c) => !c.safe).sort((a, b) => b.share - a.share)[0];

  // Reduced rather than sorted-and-indexed: `bandBreakdown` maps over the four
  // TARGET_GROUPS so it is never empty, and `reduce` says that in the type
  // instead of asserting past an index that could be undefined.
  const weakestBand = bands.reduce((worst, band) =>
    band.safeRate < worst.safeRate ? band : worst,
  );
    ...
    <div className="space-y-5">
          ...
          {choices.map((choice) => (
            <li key={choice.key}>
              ...
              <div className="mt-1.5 h-2.5 w-full overflow-hidden rounded-full bg-surface-sunk">
                <div
                  className={`h-full rounded-full ${choice.safe ? 'bg-leaf-600' : 'bg-coral-600'}`}
      ...
      {topDistractor && topDistractor.share >= 25 && (
        <div className="flex gap-3 rounded-[10px] border border-amber-200 bg-amber-50 p-4">
          ...
          <p className="text-[13px] leading-relaxed text-amber-800">
            ...
            When one unsafe option pulls this hard, the fix is usually in how that option is worded
            — not in adding another warning sign. Open <em>Review content</em> to see it in context.
          </p>
        </div>
      )}
      ...
      <p className="flex items-start gap-2 rounded-[10px] border border-line bg-surface-sunk p-3.5 text-[12px] leading-relaxed text-ink-muted">
        ...
        <span>
          ...
          participant identifier and no way to reach one from here — by design, not by policy alone.
