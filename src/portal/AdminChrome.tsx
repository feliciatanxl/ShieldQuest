...
import type { AdminSection as SectionId, Insight, PortalSummary } from './types.ts';
        ...
        {Icon && (
          <span
            ...
            className={`grid h-8 w-8 place-items-center rounded-[6px] ${
              isAttention ? 'bg-amber-50 text-amber-700' : 'bg-surface-sunk text-ink-soft'
            }`}
          >
...
export function InsightCard({ insight }: { insight: Insight }) {
  ...
  return (
    <div className={`rounded-[16px] border p-4 ${tone}`}>
      ...
      <h3 className="mt-2 text-[15px] font-bold text-navy-900">{insight.subject}</h3>
      <p className="mt-1 text-2xl font-extrabold tracking-tight text-navy-900 tabular-nums">
        {insight.value}
      </p>
      <p className="mt-1.5 text-[12.5px] leading-relaxed text-ink-muted">{insight.note}</p>
    </div>
  );
}
  ...
  return (
    <div className="mt-3 flex items-start gap-2.5 rounded-[10px] border border-amber-200 bg-amber-50 px-4 py-3">
      ...
      <p className="text-[12.5px] leading-relaxed text-amber-700">
        {children ?? (
          <>
            <strong className="font-bold">Simulated session data.</strong> Content analytics only —
            no individual responses, participants or risk scores are shown anywhere in this portal.
          </>
        )}
      </p>
    </div>
  ...
  return (
    <div className="rounded-[16px] border border-line bg-surface p-5">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line pb-4">
        <div>
          <h3 className="text-[15px] font-extrabold text-navy-900">Data safeguards &amp; ethics</h3>
          <p className="mt-0.5 text-[13px] text-ink-muted">
            How Project SHIELD handles information and why this build is safe to explore.
          </p>
        </div>
        {onResetDemo && (
          <button
